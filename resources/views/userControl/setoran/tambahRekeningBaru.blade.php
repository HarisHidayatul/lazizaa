        ...
        <div class="d-flex justify-content-center">
            <div style="width: 350px; margin: 0 10px;">
                ...
                <div class="d-flex justify-content-between">
                    <div class="pengirimLabel">Tambah Rekening</div>
                    <div class="semuaLabel" onclick="showSearchAllBank();">Semua</div>
                </div>
                <div id="listBank"></div>
    ...
    <div id="allBank">
        <div class="d-flex justify-content-center">
            <div style="width: 350px; margin: 0 10px;">
                <div style="content: ''; height: 40px;"></div>
                <div class="d-flex justify-content-between">
                    <div class="pengirimLabel">Semua Bank</div>
                </div>
    ...
    <div id="addSetoranId">
        <div class="d-flex justify-content-center">
            <div style="width: 350px; margin: 0 10px;">
                ...
                <input type="number" placeholder="Contoh: 14318161581" id="nomorRekening">
                <div class="labelInput">A.N. Rekening</div>
                <input type="text" placeholder="Contoh: Nama pemilik Rekening" id="namaRekening">
...
<script>
    ...
    function searchNama() {
        ...
        for (var i = 0; i < objAllPenerima.pengirimListArray.length; i++) {
            ...
            if (objAllPenerima.pengirimListArray[i].namaRekening.toUpperCase().indexOf(nama) > -1) {
                dataListBank += '<div class="d-flex justify-content-start rowTransaksi" onclick="goToKirimTransfer(' +
                    objAllPenerima.pengirimListArray[i].id + ')">';
    ...
    function getAllPenerimaa() {
        $.ajax({
            url: "{{ url('setoran/show/pengirim/transfer/all') }}" + '/' + "{{ session('idPengisi') }}",
            type: 'get',
                ...
                for (var i = 0; i < obj.listBank.length; i++) {
                    if (i < 4) {
                        dataListBank1 +=
                            ...
                            obj.listBank[i].id +
                            ');">';
                        dataListBank1 += '<div class="logoBankWrap">';
                        dataListBank1 += '<img src="' + url + '/' + obj.listBank[i].img + '" alt="">';
                        dataListBank1 += '</div><div class="tambahEWalletLabel">';
                        dataListBank1 += obj.listBank[i].bank;
                        dataListBank1 += '</div></div>';
                    } else {
                        dataListBank2 +=
                            '<div class="d-flex justify-content-start wrapTambahEWallet" onclick="showSetoranAddId(' +
                            obj.listBank[i].id +
                            ');">';
                        dataListBank2 += '<div class="logoBankWrap">';
                        dataListBank2 += '<img src="' + url + '/' + obj.listBank[i].img + '" alt="">';
                        dataListBank2 += '</div><div class="tambahEWalletLabel">';
                        dataListBank2 += obj.listBank[i].bank;
                        dataListBank2 += '</div></div>';
                    }
                }
                document.getElementById('listBank').innerHTML = dataListBank1;
                document.getElementById('listBankAll').innerHTML = dataListBank1 + dataListBank2;
            },
            error: function(req, err) {
                console.log(err);
            }
    ...
    function getListBankInPart() {
        $.ajax({
            url: "{{ url('setoran/show/pengirim/transfer/inPart') }}" + '/' + "{{ session('idPengisi') }}",
            type: 'get',
            success: function(response) {
                var obj = JSON.parse(JSON.stringify(response));
                console.log(obj);
                ...
                for (var i = 0; i < obj.pengirimListArray.length; i++) {
                    pengirimHTML +=
                        '<div class="wrapPengirimBank" onclick="goToKirimTransfer(' + obj.pengirimListArray[
                            i].id +
                        ')"><div style="height: 10px;"></div><div class="pengirimWrapLogoBank">';
    ...
    function goToKirimTransfer(idPengirim) {
        window.location.href = "{{ url('user/setoran/transfer/kirim/tambah') }}" + '/' + idPengirim;
    }
