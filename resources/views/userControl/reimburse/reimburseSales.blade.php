...
<body>
    <div class="fixed-top header">
        <div class="d-flex justify-content-between menuAll">
            ...
            <h4 class="kembali">Reimburse Sales</h4>
            <div></div>
        </div>
    </div>
    <div class="d-flex justify-content-center" style="margin-left: 5px; margin-right: 5px; margin-top:100px;">
        <div id="home">
            ...
            <textarea id="textAreaPesan" placeholder='"Untuk keperluan apa?"' disabled></textarea>
...
<script>
    ...
    function sendDataKirim() {
        $.ajax({
            url: "{{ url('reimburse/sales/store/data') }}",
            type: 'get',
            data: {
                idOutlet: "{{ session('idOutlet') }}",
                tanggal: currentDateMonthYear,
                total: parseInt(inputJumlah.rawValue),
                idPengisi: "{{ session('idPengisi') }}"
            },
            success: function(response) {
                console.log(response);
                var obj = JSON.parse(JSON.stringify(response));
                window.location.href = "{{ url('user/reimburse/sales/detail') }}" + '/' + obj.idSalesReimburse;
            },
