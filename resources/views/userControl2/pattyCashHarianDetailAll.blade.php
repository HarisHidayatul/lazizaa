...
<body>
    ...
    <div class="d-flex justify-content-center">
        <div class="container1">
            <div class="d-flex justify-content-center revHeader">Laporan Pembelian</div>
...
<script>
    ...
    function getAllData() {
        $.ajax({
            url: "{{ url('pattyCash/user/showAllSesi') }}" + '/' + "{{ session('idOutlet') }}" + '/' +
                "{{ $dateSelect }}",
            type: 'get',
            success: function(response) {
                var obj = JSON.parse(JSON.stringify(response));
                console.log(obj);
                var imgURL = "{{ url('img/dashboard/laporanPattyCash.png') }}";
                var dataHTML = '';
                for (var i = 0; i < obj.length; i++) {
                    var tempHTML = '';
                    var dataFound = false;
                    for (var j = 0; j < obj[i].dataPattyCash[0][0].pattyCash.length; j++) {
                        dataFound = true;
                        tempHTML +=
                            '<div class="d-flex justify-content-between boxDetail" onclick="goToDetailSesi(' +
                            obj[i].idSesi + ');">';
                        ...
                        tempHTML += '<div class="d-flex justify-content-start detailTitle"><div>' + obj[i]
                            .dataPattyCash[0][0].pattyCash[j].item + '</div>';

                        if ((obj[i].dataPattyCash[0][0].pattyCash[j].idRevQty == 2) || (obj[i]
                                .dataPattyCash[0][0].pattyCash[j].idRevTotal == 2)) {
                            var urlImage = "{{ url('img/icon/tertunda.png') }}";
                            tempHTML += '<img src="' + urlImage +
                                '" style="height: 18px" alt="icon status">';
                        } else if ((obj[i].dataPattyCash[0][0].pattyCash[j].idRevQty == 3) || (obj[i]
                                .dataPattyCash[0][0].pattyCash[j].idRevQty == 3)) {
                        ...
                        tempHTML += '</div>';
                        tempHTML += '<div class="detailSubTitle">' + obj[i].dataPattyCash[0][0].pattyCash[j]
                            .namaPengisi + '</div></div></div><div>';
                        tempHTML += '<div style="content: ' + "''" + ';height: 3px;"></div>';
                        tempHTML += '<div class="detailRev" style="color: #008000;">' +
                            obj[i].dataPattyCash[0][0].pattyCash[j].qty +
                            ' ' +
                            obj[i].dataPattyCash[0][0].pattyCash[j].satuan +
                            '<span style="color: #B20731;">';
    ...
    function goToDetailSesi(index) {
        window.location.href = "{{ url('user/detail/pattyCashHarian') }}" + '/' + "{{ $dateSelect }}" + '/' + index;
    }
</script>
