...
<script>
    ...
    function goToEdit() {
        window.location.href = "{{ url('user/salesHarian') }}" + "/" + "";
    }

    function showAllData() {
        $.ajax({
            url: "{{ url('salesHarian/show/salesFill') }}" + '/' + "{{ $idSalesFill }}",
            type: 'get',
            success: function(response) {
                ...
                for (var i = 0; i < obj.length; i++) {
                    ...
                    for (var j = 0; j < obj[i].sales.length; j++) {
                        ...
                        if (indexRow == 0) {
                            namaPengisi1 = obj[i].sales[j].namaPengisi[0];
                        }
                        if (indexRow == 1) {
                            namaPengisi2 = obj[i].sales[j].namaPengisi[0];
                        }
                        if (indexRow == 2) {
                            namaPengisi3 = obj[i].sales[j].namaPengisi[0];
                        }
                        indexRow++;
                    }
                    dataFill += '</div>';
