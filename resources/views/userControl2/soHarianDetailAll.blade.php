...
<script>
    ...
    function getAllData() {
        $.ajax({
            url: "{{ url('soHarian/user/showAllSesi') }}" + '/' + "{{ session('idOutlet') }}" + '/' +
                "{{ $dateSelect }}",
            type: 'get',
            success: function(response) {
                var obj = JSON.parse(JSON.stringify(response));
                console.log(obj);
                var imgURL = "{{ url('img/dashboard/laporanSo.png') }}";
                var dataHTML = '';
                for (var i = 0; i < obj.length; i++) {
                    var tempHTML = '';
                    var dataFound = false;
                    for (var j = 0; j < obj[i].dataSoHarian[0].length; j++) {
                        dataFound = true;
                        ...
                        tempHTML += '<div><img src="' + imgURL + '" alt="" style="height: 25px; margin-top:5px;"></div>';
                        tempHTML += '<div style="margin-left: 10px; margin-top:3px;">';
                        tempHTML += '<div class="d-flex justify-content-start detailTitle"><div>' + obj[i].dataSoHarian[0][j].Item + '</div>';

                        if (obj[i].dataSoHarian[0][j].idRevisi == 2) {
                            var urlImage = "{{ url('img/icon/tertunda.png') }}";
                            tempHTML += '<img src="' + urlImage + '" style="height: 18px" alt="icon status">';
                        } else if (obj[i].dataSoHarian[0][j].idRevisi == 3) {
                        ...
                        tempHTML += '</div>';
                        tempHTML += '<div class="detailSubTitle">' + obj[i].dataSoHarian[0][j].namaPengisi + '</div></div></div><div>';
                        tempHTML += '<div style="content: '+"''"+';height: 6px;"></div>';
                        tempHTML += '<div class="detailRev"><span style="color: #0000FF;">';
                        tempHTML += obj[i].dataSoHarian[0][j].quantity;
                        tempHTML += ' ';
                        tempHTML += obj[i].dataSoHarian[0][j].satuan;
                        tempHTML += '</span></div>';
                        tempHTML += '<div class="detailRev"><span style="color: #FFA500;">';
                        // tempHTML += obj[i].dataSoHarian[0][j].totalQty.toLocaleString();
    ...
    function goToDetailSesi(index){
        window.location.href = "{{ url('user/detail/soHarian') }}" + '/' + "{{ $dateSelect }}" + '/' + index;
    }
</script>
