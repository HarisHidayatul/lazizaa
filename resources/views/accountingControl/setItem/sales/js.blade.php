...
@section('setItemJS')
    ...
    <script>
        $(document).ready(function() {
            var topNavBar = [
                ["List Item", "{{ url('accounting/sales/item') }}"],
                ["Set Type", "{{ url('accounting/sales/setType') }}"],
                ["Outlet Type", "{{ url('accounting/sales/outletType') }}"],
                ["Pending Item","{{ url('accounting/sales/pendingItem') }}"]
            ];
            var topNavHTML = '';
