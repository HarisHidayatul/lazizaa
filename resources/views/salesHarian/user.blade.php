...
<body>
    ...
    {{-- {{ $idPengisi }} --}}
    <img src="{{ $brandImage }}" width="150" height="150">
    ...
    <script>
        ...
        var valueCuEdit = new AutoNumeric('#editCU', { decimalPlaces: '0' });
        var valueTotalEdit = new AutoNumeric('#editTotal', { decimalPlaces: '0' });
        ...
        function refreshFillTable() {
            $.ajax({
                url: "{{ 'salesHarian/user/showTable/' }}" + "{{ $idOutlet }}" + '/' + dates,
                type: 'get',
                success: function(response) {
                    // console.log(response);
                    dataSales.length = 0;
        ...
        function getItemOnOutlet() {
            $.ajax({
                url: "{{ url('salesHarian/show/list/') }}" + '/' + {{ $idOutlet }},
                type: 'get',
                success: function(response) {
                    // console.log(response);
                    var obj = JSON.parse(JSON.stringify(response));
                    ...
                    for(var i =0;i<row;i++){
                        var idRow = '#r' + i + 'c1';
                        valueTotalAll.push(new AutoNumeric(idRow, { decimalPlaces: '0' }));
                    }
                    refreshFillTable();
                },
                error: function(req, err) {
